...
Использование интерфейса `Function<T, R>` позволяет передавать функцию как аргумент в другие методы, возвращать ее из методов.
Так же методы по умолчанию могут использоваться для построения цепочек вызовов (`compose`, `andThen`).
